...
export default {
  ...
  preview: {
    ...
    exportedStep: "已导出 3 个 STEP 到 {dir}",
  ...
  config: {
    ...
    warnings: {
      title: "参数提醒",
      stencilTooSmall: "钢网边长小于 PCB 尺寸,印刷区可能覆盖不全",
      jigTooSmall: "夹具边长不足:钢网 {s}mm 建议配 {j}mm 夹具,否则窗口/螺丝孔过于贴近边缘",
      pcbTooBig: "PCB({p}mm)超出夹具({j}mm)的可容纳范围",
      insertTooThin: "插板厚 {h}mm 不足以开出 PCB 槽(至少需要 {n}mm)",
      postTooSmall: "螺柱直径 {d}mm 偏小,M3 螺丝自攻壁厚不足",
      headTooSmall: "沉孔直径 {d}mm 与过孔 {c}mm 太接近,沉头螺母会失去作用",
      coverTooThin: "顶盖厚 {h}mm 偏薄,螺母沉孔(深 1.5mm)可能穿透",
      supportOutside: "支撑柱(偏移 {o}mm)可能落在 PCB 板框外,起不到支撑作用",
    },
  },
  python: {
    // 状态
